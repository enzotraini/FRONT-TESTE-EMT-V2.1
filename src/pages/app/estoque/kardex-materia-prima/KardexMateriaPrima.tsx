...
export function KardexMateriaPrima() {
	...
	// Mock data - Matéria Prima
	const { data: materiasResponse, isFetching: isFetchingMaterias } = useQuery({
		queryKey: ["materias-kardex", searchMateria],
		queryFn: async () => {
			...
			const mockMaterias: MateriaPrima[] = [
				...
				},
				{
					id: 5,
					codigo: "MP005",
					descricao: "CABO FLAT IDE",
					unidade: "UN",
					tipo: "ACESSORIO",
					saldoAtual: 80,
					custoMedio: 8.50,
					valorEstoque: 680.00,
					ultimaMovimentacao: "2025-01-11T11:30:00"
				}
			];

			const filteredMaterias = searchMateria 
				? mockMaterias.filter(m => 
					m.descricao.toLowerCase().includes(searchMateria.toLowerCase()) ||
					m.codigo.includes(searchMateria) ||
					m.referencia?.toLowerCase().includes(searchMateria.toLowerCase())
				)
				: mockMaterias;

			return { materias: filteredMaterias };
		},
		staleTime: 1000 * 60 * 5,
	});

	// Mock data - Movimentações da matéria prima selecionada
	const { data: movimentacoesResponse, isFetching: isFetchingMovimentacoes } = useQuery({
		queryKey: ["movimentacoes-materia", materiaSelecionada?.id, tipoFiltro],
		queryFn: async () => {
			if (!materiaSelecionada) return { movimentacoes: [] };
			...
			const mockMovimentacoes: MovimentacaoKardex[] = [
				...
				},
				{
					...
					documento: "NF-234",
					cliforn: "FORNECEDOR ABC LTDA",
					tipo: "ENTRADA COMPRA",
					observacao: "Compra de matéria prima",
					quantidade: 100,
					saldoAnterior: 160,
					saldoAtual: 260,
					custoUnitario: 15.00,
				...
				},
				{
					...
					observacao: "Ajuste por inventário físico",
					quantidade: 5,
					saldoAnterior: 155,
					saldoAtual: 160,
					custoUnitario: 15.50,
			...
			const filteredMovimentacoes = tipoFiltro === "todos" 
				...
				: mockMovimentacoes.filter(m => {
					switch(tipoFiltro) {
						...
						case "ajustes": return m.tipo.includes("AJUSTE");
						case "producao": return m.tipo.includes("PRODUCAO");
						default: return true;
					}
				});
		...
		enabled: !!materiaSelecionada,
		staleTime: 1000 * 60 * 2,
	});

	const handleSubmitMovimentacao = async (values: z.infer<typeof movimentacaoSchema>) => {
		console.log("Nova movimentação:", values, "Matéria Prima:", materiaSelecionada);
	...
	const getTipoColor = (tipo: string) => {
		...
		if (tipo.includes("AJUSTE")) return "text-blue-700 bg-blue-50";
		if (tipo.includes("PRODUCAO")) return "text-purple-700 bg-purple-50";
	...
	const getTipoIcon = (tipo: string) => {
		...
		if (tipo.includes("AJUSTE")) return <RotateCcw className="h-3 w-3" />;
		if (tipo.includes("PRODUCAO")) return <RefreshCw className="h-3 w-3" />;
	...
	const getTipoMateriaColor = (tipo: string) => {
		if (tipo === "COMPONENTE") return "bg-blue-100 text-blue-800";
		if (tipo === "ACESSORIO") return "bg-green-100 text-green-800";
		...
		<section className="flex flex-col w-full max-h-screen">
			...
			<header className="flex gap-3 items-center justify-between p-4 bg-gray-50 border-b border-gray-200 dark:bg-gray-950 dark:border-gray-800">
				<div className="flex gap-3">
					...
					<Button variant="green" disabled={!materiaSelecionada}>
						<TrendingUp className="h-4 w-4 mr-2" />
						Entrada
					</Button>
					<Button variant="outline" disabled={!materiaSelecionada}>
						<TrendingDown className="h-4 w-4 mr-2" />
						Saída
					</Button>
					<Button variant="ghost" disabled={!materiaSelecionada}>
			...
			<div className="flex flex-1 overflow-hidden">
				...
				<div className="w-1/3 flex flex-col bg-white border-r border-gray-200 p-4">
					<div className="mb-4">
						<h2 className="text-lg font-semibold text-gray-900 mb-2">Selecionar Matéria Prima</h2>
						<SearchInput
					...
					<div className="flex-1 overflow-auto">
						{isFetchingMaterias ? (
						...
						) : !materiasResponse?.materias || materiasResponse.materias.length === 0 ? (
							...
							<div className="space-y-2">
								{materiasResponse.materias.map((materia) => (
									<Card
										key={materia.id}
										className={`cursor-pointer transition-colors hover:bg-gray-50 ${
											materiaSelecionada?.id === materia.id ? "ring-2 ring-blue-500 bg-blue-50" : ""
										}`}
										onClick={() => setMateriaSelecionada(materia)}
									>
										<CardContent className="p-4">
											<div className="flex justify-between items-start mb-2">
												<div className="flex-1">
													<p className="font-medium text-sm">{materia.codigo} - {materia.descricao}</p>
													{materia.referencia && (
														<p className="text-xs text-gray-500">Ref: {materia.referencia}</p>
													)}
													<Badge className={`text-xs mt-1 ${getTipoMateriaColor(materia.tipo)}`}>
														{materia.tipo}
													</Badge>
												</div>
												<div className="text-right">
													<div className={`flex items-center gap-1 text-sm font-medium ${getSaldoStatus(materia.saldoAtual).color}`}>
														{getSaldoStatus(materia.saldoAtual).icon}
														{materia.saldoAtual} {materia.unidade}
													</div>
												</div>
											</div>
											<div className="flex justify-between text-xs text-gray-600">
												<span>Custo: {formatarMoeda(materia.custoMedio)}</span>
												<span>Total: {formatarMoeda(materia.valorEstoque)}</span>
											</div>
										</CardContent>
									</Card>
								))}
				...
				{/* Movimentações da Matéria Prima */}
					...
					{materiaSelecionada ? (
							...
							<div className="p-4 border-b border-gray-200 bg-gray-50">
								<div className="flex items-start justify-between mb-3">
									<div>
										<h2 className="text-xl font-semibold text-gray-900">
											{materiaSelecionada.codigo} - {materiaSelecionada.descricao}
										</h2>
										{materiaSelecionada.referencia && (
											<p className="text-gray-600">Referência: {materiaSelecionada.referencia}</p>
										)}
										<Badge className={`mt-2 ${getTipoMateriaColor(materiaSelecionada.tipo)}`}>
											{materiaSelecionada.tipo}
										</Badge>
									</div>
									<div className={`flex items-center gap-2 ${getSaldoStatus(materiaSelecionada.saldoAtual).color}`}>
										{getSaldoStatus(materiaSelecionada.saldoAtual).icon}
										<span className="font-semibold">Saldo: {materiaSelecionada.saldoAtual} {materiaSelecionada.unidade}</span>
									</div>
								</div>
								...
								<div className="grid grid-cols-3 gap-4 text-sm">
									<div>
										...
										<p className="font-medium text-lg">{formatarMoeda(materiaSelecionada.custoMedio)}</p>
									</div>
									<div>
										<span className="text-gray-500">Valor Estoque:</span>
										<p className={`font-medium text-lg ${materiaSelecionada.valorEstoque < 0 ? 'text-red-600' : 'text-green-600'}`}>
											{formatarMoeda(materiaSelecionada.valorEstoque)}
										</p>
									</div>
									<div>
										<span className="text-gray-500">Última Movimentação:</span>
										<p className="font-medium">
											{materiaSelecionada.ultimaMovimentacao 
												? format(new Date(materiaSelecionada.ultimaMovimentacao), "dd/MM/yyyy", { locale: ptBR })
												: "Sem movimentação"
											}
										</p>
									</div>
								...
								<Table>
									<TableHeader className="sticky top-0 bg-white border-b border-gray-200">
										<TableRow>
											...
											<TableHead className="w-60">Origem/Destino</TableHead>
						...
						<div className="flex-1 flex items-center justify-center text-gray-500">
							<div className="text-center">
								...
								<h3 className="text-lg font-medium mb-2">Selecione uma matéria prima</h3>
								<p className="text-sm text-gray-400">Escolha uma matéria prima da lista para ver o histórico de movimentações</p>
							</div>
						</div>
					)}
				</div>
				...
				<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
					<Card className="w-full max-w-md mx-4">
						<CardHeader>
							...
							<p className="text-sm text-gray-600">{materiaSelecionada.descricao}</p>
						</CardHeader>
						<CardContent>
							<Form {...form}>
											...
											<FormItem>
												...
												<Select onValueChange={field.onChange} value={field.value}>
													...
													<SelectContent>
														...
														<SelectItem value="SAIDA PRODUCAO">Saída para Produção</SelectItem>
														<SelectItem value="AJUSTE INVENTARIO">Ajuste de Inventário</SelectItem>
